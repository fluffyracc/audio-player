...
func GenerateImage(path string) ([]byte, error) {
	...
	cmd := exec.Command("ffmpeg", "-i", path, "-filter_complex", "aformat=channel_layouts=mono,compand,showwavespic=s="+strconv.Itoa(width)+"x"+strconv.Itoa(height)+":colors=red:scale=sqrt:draw=full", "-frames:v", "1", outPath)
	err = cmd.Run()
	if err != nil {
		return nil, err
	}
